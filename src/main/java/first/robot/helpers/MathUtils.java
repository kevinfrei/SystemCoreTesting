...
public class MathUtils {
    ...
    // Always returns a value between 0 and 359.999 (or 2pi, depending on AngleType)
    public static double snapToNearestAngleMultiple(
        double input,
        double angleMultiple,
        AngleType AngleType
    ) {
        int quadrant = (int) Math.round(input / angleMultiple);
        return (quadrant * angleMultiple) % fullCircle(AngleType);
    }

    public static double snapToNearestRadiansMultiple(double input, double angleMultiple) {
        return snapToNearestAngleMultiple(input, angleMultiple, AngleType.RADIANS);
    }

    public static double snapToNearestDegreesMultiple(double input, double angleMultiple) {
        return snapToNearestAngleMultiple(input, angleMultiple, AngleType.DEGREES);
    }

    public static double normalizeAngle(double angle, AngleType AngleType) {
        double twoPi = fullCircle(AngleType);
        return ((angle % twoPi) + twoPi) % twoPi;
    }

    public static double normalizeRadians(double radians) {
        return normalizeAngle(radians, AngleType.RADIANS);
    }

    public static double normalizeDegrees(double degrees) {
        return normalizeAngle(degrees, AngleType.DEGREES);
    }
     ...
     */
    public static double normalizeDeltaAngle(double angleDelta, AngleType AngleType) {
        double half = halfCircle(AngleType);
        return normalizeAngle(angleDelta + half, AngleType) - half;
    }
    ...
    public static double normalizeDeltaRadians(double angleDelta) {
        return normalizeDeltaAngle(angleDelta, AngleType.RADIANS);
    }
    ...
    public static double normalizeDeltaDegrees(double angleDelta) {
        return normalizeDeltaAngle(angleDelta, AngleType.DEGREES);
    }

    public static double fullCircle(AngleType AngleType) {
        return AngleType == AngleType.RADIANS ? Math.PI * 2 : 360;
    }

    public static double halfCircle(AngleType AngleType) {
        return AngleType == AngleType.RADIANS ? Math.PI : 180;
    }
